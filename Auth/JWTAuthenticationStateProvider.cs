...
namespace LCMSMSPWA.Auth
{
    public class JWTAuthenticationStateProvider : AuthenticationStateProvider, ILoginService
    {
        ...
        {
            var token = await localStorageService.GetItem<string>(TOKENKEY);
            if (string.IsNullOrEmpty(token))
            {
                return Anonymous;
            }

            var expirationTimeString = await localStorageService.GetItem<string>(EXPIRATIONTOKENKEY);
        ...
        private async Task<string> RenewToken(string token)
        {
            ...
            await localStorageService.SetItem(TOKENKEY, newToken.Token);
            await localStorageService.SetItem(EXPIRATIONTOKENKEY, newToken.Expiration.ToString());
        ...
        {
            await localStorageService.SetItem(TOKENKEY, userToken.Token);
            await localStorageService.SetItem(EXPIRATIONTOKENKEY, userToken.Expiration.ToString());
        ...
        {
            await localStorageService.RemoveItem(TOKENKEY);
            await localStorageService.RemoveItem(EXPIRATIONTOKENKEY);
